...
# Source .env if it exists (for SLACK_BOT_TOKEN, etc.)
...
if [[ ${CRASH_COUNT} -ge ${MAX_CRASHES_PER_DAY} ]]; then
    ...
    if [[ -n "${SLACK_BOT_TOKEN:-}" && -n "${SLACK_CHANNEL_ID:-}" ]]; then
        ALERT_TEXT="ALERT: Your agent has crashed ${MAX_CRASHES_PER_DAY} times today and has been halted. Restart with: launchctl bootout gui/\$(id -u) ~/Library/LaunchAgents/com.my-agent.plist && launchctl bootstrap gui/\$(id -u) ~/Library/LaunchAgents/com.my-agent.plist"
    ...
    # 24h silent halt removed: the independent supervisor (com.my-agent-supervisor)
    # now owns recovery policy (progressive backoff + leveled Slack alerts).
    # Exit cleanly; launchd ThrottleInterval + supervisor decide what happens next.
    exit 1
fi
